...
$therapist_name_for_pdf = is_array($therapist)
    ? ($therapist['name'] ?? $therapist['username'] ?? 'Therapist')
    : (is_object($therapist) ? ($therapist->name ?? $therapist->username ?? 'Therapist') : 'Therapist');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Therapist Dashboard - {{ $siteName }}</title>
...
<body>
    @include('therapist.partials.navbar')
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            if (!isset($pdo)) {
                                ...
                                $is_editable = false;
                                echo view('admin.partials.dynamic_timetable_grid', compact('pdo', 'is_editable', 'therapist_id') + ['baseUrl' => $baseUrl, 'siteName' => $siteName])->render();
                            }
                            ?>
                        </div>
                    </div>
    ...
    <script src="{{ $baseUrl }}assets/js/therapist.js"></script>
    <script>
        ...
        $('#personalInfoForm').on('submit', function(e) {
            ...
            $.ajax({
                url: 'api/update-therapist-profile',
                method: 'POST',
                data: formData,
                dataType: 'json',
