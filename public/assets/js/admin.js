$(document).ready(function() {
    ...
    function loadCustomTimings(therapistId) {
        return $.ajax({
            url: 'api/get-custom-timings?therapist_id=' + therapistId,
            method: 'GET',
            success: function(response) {
                var res = typeof response === 'string' ? JSON.parse(response) : response;
                if (res.success && res.data) {
    ...
    function loadExistingTimetable(therapistId) {
        $.ajax({
            url: 'api/get-timetable-grid?therapist_id=' + therapistId,
            method: 'GET',
            success: function(response) {
                var res = typeof response === 'string' ? JSON.parse(response) : response;
                if (res.success) {
    ...
    function loadTimetableGrid(therapistId) {
        ...
        $.ajax({
            url: 'api/load-dynamic-grid?therapist_id=' + therapistId,
            method: 'GET',
    ...
    function populateKidsTable(kids) {
        ...
        for (var i = 0; i < kids.length; i++) {
            ...
            rows.push('<tr><td class="fw-semibold">' + (serial++) + '</td>' +
    ...
    $(document).on('click', '.delete-timetable-entry', function() {
        ...
        $.ajax({
            url: 'api/delete-timetable-entry',
            method: 'POST',
            data: { id: id },
            success: function(response) {
                var res = typeof response === 'string' ? JSON.parse(response) : response;
