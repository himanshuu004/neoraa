...
$error = '';
$generated_by = isset($_GET['ref']) ? (int)$_GET['ref'] : null;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Ensure table exists only when submitting (keeps GET requests fast)
    try {
    ...
    $areas_specialization = isset($_POST['areas_specialization']) && is_array($_POST['areas_specialization'])
        ? implode(', ', array_map('trim', $_POST['areas_specialization'])) : '';
    ...
    if (strlen($name) < 2) {
        ...
        $upload_dir = rtrim(__DIR__, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'hiring' . DIRECTORY_SEPARATOR;
        if (!is_dir($upload_dir)) {
            if (!@mkdir($upload_dir, 0755, true)) {
                $error = 'Upload folder could not be created. Please contact support.';
            }
        }
        if ($error === '' && !is_writable($upload_dir)) {
            $error = 'Upload folder is not writable. Please contact support.';
        }
        ...
        if ($error === '' && !empty($_FILES['resume']['name'])) {
            ...
            if ($err !== UPLOAD_ERR_OK) {
                $errors = [
                    UPLOAD_ERR_INI_SIZE => 'Resume exceeds server limit.',
                    UPLOAD_ERR_FORM_SIZE => 'Resume is too large (max 5MB).',
                    UPLOAD_ERR_PARTIAL => 'Resume upload was incomplete.',
                    UPLOAD_ERR_NO_FILE => 'No resume file selected.',
                    UPLOAD_ERR_NO_TMP_DIR => 'Server upload folder missing.',
                    UPLOAD_ERR_CANT_WRITE => 'Server could not save resume.',
                    UPLOAD_ERR_EXTENSION => 'Resume upload blocked by server.',
                ];
                $error = $errors[$err] ?? 'Resume upload failed (error ' . $err . ').';
            } else {
                $ext = strtolower(pathinfo($_FILES['resume']['name'], PATHINFO_EXTENSION));
                if (!in_array($ext, $allowed_ext) || $_FILES['resume']['size'] > $max_size) {
                    $error = 'Resume: allowed PDF, DOC, DOCX; max 5MB.';
                } else {
                    $base = pathinfo($_FILES['resume']['name'], PATHINFO_FILENAME);
                    $safe_name = preg_replace('/[^a-zA-Z0-9._-]/', '_', $base);
                    $filename = 'resume_' . time() . '_' . $safe_name . '.' . $ext;
                    $dest_path = $upload_dir . $filename;
                    $resume_path = 'uploads/hiring/' . $filename;
                    if (!@move_uploaded_file($_FILES['resume']['tmp_name'], $dest_path)) {
                        $error = 'Failed to save resume. Check folder permissions (uploads/hiring).';
                    }
                }
            }
        }
        if ($error === '' && !empty($_FILES['certificate']['name'])) {
            $err = $_FILES['certificate']['error'];
            if ($err !== UPLOAD_ERR_OK) {
                $errors = [
                    UPLOAD_ERR_INI_SIZE => 'Certificate exceeds server limit.',
                    UPLOAD_ERR_FORM_SIZE => 'Certificate is too large (max 5MB).',
                    UPLOAD_ERR_PARTIAL => 'Certificate upload was incomplete.',
                    UPLOAD_ERR_NO_FILE => 'No certificate file selected.',
                    UPLOAD_ERR_NO_TMP_DIR => 'Server upload folder missing.',
                    UPLOAD_ERR_CANT_WRITE => 'Server could not save certificate.',
                    UPLOAD_ERR_EXTENSION => 'Certificate upload blocked by server.',
                ];
                $error = $errors[$err] ?? 'Certificate upload failed (error ' . $err . ').';
            } else {
                $ext = strtolower(pathinfo($_FILES['certificate']['name'], PATHINFO_EXTENSION));
                if (!in_array($ext, $allowed_ext) || $_FILES['certificate']['size'] > $max_size) {
                    $error = 'Certificate: allowed PDF, DOC, DOCX; max 5MB.';
                } else {
                    $base = pathinfo($_FILES['certificate']['name'], PATHINFO_FILENAME);
                    $safe_name = preg_replace('/[^a-zA-Z0-9._-]/', '_', $base);
                    $filename = 'cert_' . time() . '_' . $safe_name . '.' . $ext;
                    $dest_path = $upload_dir . $filename;
                    $certificate_path = 'uploads/hiring/' . $filename;
                    if (!@move_uploaded_file($_FILES['certificate']['tmp_name'], $dest_path)) {
                        $error = 'Failed to save certificate. Check folder permissions (uploads/hiring).';
                    }
                }
            }
        }
        ...
        if ($error === '') {
            try {
                $stmt = $pdo->prepare("INSERT INTO hiring_applications (name, mobile, email, city, applying_for, qualification, college, year, experience_type, experience_years, current_place, areas_specialization, languages, joining_time, resume_path, certificate_path, why_join_neora, generated_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$name, $mobile, $email, $city, $applying_for, $qualification, $college, $year, $experience_type, $experience_years, $current_place, $areas_specialization, $languages, $joining_time, $resume_path, $certificate_path, $why_join_neora, $generated_by]);
                header('Location: ' . BASE_URL . 'apply-confirmation.php');
                exit();
            } catch (PDOException $e) {
...
<head>
    ...
    <title>Apply - <?php echo SITE_NAME; ?></title>
    ...
    <div class="max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto px-3 sm:px-6 lg:px-10 xl:px-12 py-5 sm:py-8 lg:py-12 w-full min-w-0">
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden mb-5 sm:mb-8">
            <div class="bg-white px-4 sm:px-6 lg:px-10 xl:px-12 py-6 sm:py-8 lg:py-10 text-center border-b border-slate-200/80">
                <img src="<?php echo BASE_URL; ?>uploads/logo.png" alt="Logo" class="h-12 sm:h-14 lg:h-16 mx-auto object-contain w-auto">
        ...
        <form id="apply-form" method="post" enctype="multipart/form-data" class="space-y-4 sm:space-y-6 lg:space-y-8">
            <!-- Basic Details -->
