...
if (isset($_GET['redirected'])) {
    // Clear session if we're in a redirect loop
    session_destroy();
    session_start();
}

// Redirect if already logged in (save session first so next request sees it)
if (isLoggedIn() && !isset($_GET['redirected'])) {
    session_write_close();
    $redirectUrl = '';
    if (isAdmin()) {
        $redirectUrl = BASE_URL . 'admin/dashboard.php';
    } elseif (isCoordinator()) {
        $redirectUrl = BASE_URL . 'coordinator/dashboard.php';
    } elseif (isTrainee()) {
        $redirectUrl = BASE_URL . 'trainee/dashboard.php';
    } else {
        $redirectUrl = BASE_URL . 'therapist/dashboard.php';
    }
...
if (!isLoggedIn() && isset($_COOKIE['remember_username']) && isset($_COOKIE['remember_user_type'])) {
    $saved_username = $_COOKIE['remember_username'];
    $saved_user_type = $_COOKIE['remember_user_type'];
} else {
    $saved_username = '';
    $saved_user_type = 'admin';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        try {
            ...
            $role_name = $user_type === 'admin' ? 'admin' : ($user_type === 'coordinator' ? 'coordinator' : ($user_type === 'trainee' ? 'trainee' : 'therapist'));
            $stmt = $pdo->prepare("SELECT r.role_name, u.id, u.username, u.password 
                                   FROM users u 
                                   JOIN roles r ON u.role_id = r.id 
                                   WHERE u.username = ? AND r.role_name = ?");
            $stmt->execute([$username, $role_name]);
            $user = $stmt->fetch();
            ...
            if ($user) {
                ...
                if ($passwordMatch) {
                    ...
                    // Handle Remember Me
                    if ($remember_me) {
                        // Set cookies for 30 days (path = BASE_URL so cookie is scoped to app)
                        setcookie('remember_username', $username, time() + (30 * 24 * 60 * 60), BASE_URL);
                        setcookie('remember_user_type', $user_type, time() + (30 * 24 * 60 * 60), BASE_URL);
                    } else {
                        setcookie('remember_username', '', time() - 3600, BASE_URL);
                        setcookie('remember_user_type', '', time() - 3600, BASE_URL);
                    }
                    
                    // Save session before redirect so next request sees it (prevents redirect loop)
                    session_write_close();
                    if ($user['role_name'] === 'admin') {
                        header('Location: ' . BASE_URL . 'admin/dashboard.php');
                    } elseif ($user['role_name'] === 'coordinator') {
                        header('Location: ' . BASE_URL . 'coordinator/dashboard.php');
                    } elseif ($user['role_name'] === 'trainee') {
                        header('Location: ' . BASE_URL . 'trainee/dashboard.php');
                    } else {
                        header('Location: ' . BASE_URL . 'therapist/dashboard.php');
                    }
                    exit();
                } else {
                ...
                $error = 'User not found. Please check username and user type.';
            }
        } catch (Exception $e) {
...
<head>
    ...
    <!-- Main design system -->
    <link rel="stylesheet" href="<?php echo BASE_URL; ?>assets/css/neora-redesign.css">
    ...
    <style>
        ...
        .nd-login-brand-img {
            ...
            background: url('public/landing/sliders/494bd119-204d-4c5f-9b19-29fb0ae98677.JPG') center center / cover no-repeat;
...
<body class="nd-login-body">
    ...
    <div class="nd-login-form-panel">
        <div class="nd-login-form-inner" data-aos="fade-up" data-aos-duration="700">
            ...
            <form method="POST" action="<?php echo htmlspecialchars(BASE_URL . 'login.php'); ?>" id="loginForm">
            ...
            <p class="nd-login-footnote">
                ...
                <br>
                <a href="<?php echo BASE_URL; ?>" style="color:var(--primary-color);text-decoration:none;font-weight:600;">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Pre-fill from localStorage if cookies not present
        if (!$('#username').val()) {
            var saved = localStorage.getItem('neora_username');
            if (saved) $('#username').val(saved);
        }
        var savedType = localStorage.getItem('neora_user_type');
        if (savedType && $('#user_type').val() === 'admin') $('#user_type').val(savedType);

        // On submit — save to localStorage if remember checked
        $('#loginForm').on('submit', function () {
            var username  = $('#username').val();
            var userType  = $('#user_type').val();
            var remember  = $('#remember_me').is(':checked');
            if (remember) {
                localStorage.setItem('neora_username',  username);
                localStorage.setItem('neora_user_type', userType);
            } else {
                localStorage.removeItem('neora_username');
                localStorage.removeItem('neora_user_type');
            }
            localStorage.removeItem('neora_password');
        });
    });
    </script>
</body>
